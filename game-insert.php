...
$stmt = $mysqli->prepare("SELECT gameID, gameName, gameDescription, gameCertificate, gameImage, gameDeveloper, platformID, metacriticScore, releaseDate, gameIcon, twitterID, twitterURL, gameSummary FROM games WHERE gameID = ?");
$stmt->bind_param('i', $_GET['gameID']);
$stmt->execute(); 
$stmt->bind_result($gameID, $gameName, $gameDescription, $gameCertificate, $gameImage, $gameDeveloper, $platformID, $metacriticScore, $releaseDate, $gameIcon, $twitterID, $twitterURL, $gameSummary);
$stmt->fetch();
$stmt->close();
?>

<p><a href="list-all-games.php" class="button-style">&lt;&lt; Back to List</a>
<h1 class="center">Insert New Game</h1></p>
...
<!-- Form Start -->
<form name="form1" method="post" action="insertGameRecord.php">
...
<input name="gameID" type="hidden" value="<?php echo $gameID; ?>">
 ...
 <p class="indent">
  ...
  <?php
  echo "<option value ='1'>$platformName</option>";
